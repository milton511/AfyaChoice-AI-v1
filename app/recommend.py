...
def rank_methods(user_data, user_preference=None):
    ...
    for m in methods:
        ...
        if privacy == "Prefers private methods (implant / IUD / injectable)":
            if "Implant" in m["name"] or "IUD" in m["name"] or "Injectables" in m["name"]:
                pref_weight = 25
        elif privacy == "Prefers pills (may be less discreet)":
            if "Pill" in m["name"]:
                pref_weight = 25
        ...
        if pref_weight > 0:
            explanation_parts.append("Fits your privacy preference")
